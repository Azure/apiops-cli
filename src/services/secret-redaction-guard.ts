...
import { readPolicyFragmentArtifactDetails } from './policy-fragment-artifact.js';
...
async function scanPolicy(
  ...
  const fragmentArtifact = descriptor.type === ResourceType.PolicyFragment
    ? await readPolicyFragmentArtifactDetails(
        store,
        config.sourceDir,
        descriptor
      )
    : undefined;
  const payload = fragmentArtifact?.payload ??
  ...
  if (typeof mergedValue === 'string' && mergedValue.includes(REDACTION_MARKER)) {
    const location = descriptor.type === ResourceType.PolicyFragment
      ? getPolicyFragmentValueLocation(
          descriptor,
          config,
          fragmentArtifact?.valueSource
        )
      : 'policy.xml';
    return {
      descriptor,
...
function getPolicyFragmentValueLocation(
  descriptor: ResourceDescriptor,
  config: PublishConfig,
  artifactSource: 'policy.xml' | 'policyFragmentInformation.json' | undefined
): string {
  ...
  if (
    hasExplicitPropertyOverride(
      name,
      'value',
      config.overrides?.policyFragments
    )
  ) {
    return `overrides.policyFragments.${name}.properties.value`;
  }

  return artifactSource === 'policyFragmentInformation.json'
