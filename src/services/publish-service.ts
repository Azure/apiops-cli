...
import { trackRetries } from '../lib/retry-tracker.js';
import {
  getApiRootName,
  getNamePart,
...
/**
 * Human-readable labels for each dependency tier, used in publish output headers.
 */
...
async function deleteTier(
  ...
  const tasks = descriptors.map((descriptor) => async () => {
    ...
    return retries > 0 ? { ...value, retries } : value;
  });
...
/**
 * Attribute the retries recorded for one publish task to its primary result
 * (the first result returned for the descriptor).
 */
function attachRetries(results: PublishActionResult[], retries: number): PublishActionResult[] {
  if (retries === 0 || results.length === 0) {
    return results;
  }
  const [primary, ...rest] = results;
  return [{ ...primary, retries }, ...rest];
}
